...
The scanner deliberately does **not** infer that every flare lacking a physical `[FlareMineClass]` must crash; that stronger absence claim would require direct proof of the relevant prototype/default state.

## `baseName` semantics

A previous scanner revision modeled canonical `baseName` as an ODF-to-ODF inheritance edge. Loader mining disproved that model.
...
Lowercase `basename` remains distinct from canonical `baseName`; the scanner does not invent global key case-insensitivity.
